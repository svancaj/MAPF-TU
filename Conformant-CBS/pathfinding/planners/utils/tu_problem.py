...
class TimeUncertaintyProblem:
    ...
    def print_picat_instance(self, picat_file):

        with open(picat_file, 'w') as file:
            ...
            file.write("get_instance() = I =>\n")
            file.write("    I = $instance(\n")
            file.write("    [\n")

            for v in self.edges_and_weights:
                (v_x,v_y) = v
                file.write("    $neibs(" + str(local_map[v_x][v_y]) + ", [(" + str(local_map[v_x][v_y]) + ",1,1)")
                for n in self.edges_and_weights[v]:
                    ((n_x,n_y),(LU,UU)) = n
                    file.write(",(" + str(local_map[n_x][n_y]) + "," + str(LU) + "," + str(UU) + ")")
                if v == list(self.edges_and_weights)[-1]:
                    file.write("])\n")
                else:
                    file.write("]),\n")
            file.write("    ],\n")
            ...
            file.write("    [")
            for a in self.start_positions:
                (start_x,start_y) = self.start_positions[a]
                (goal_x,goal_y) = self.goal_positions[a]
                file.write("(" + str(local_map[start_x][start_y]) + "," + str(local_map[goal_x][goal_y]) + ")")
                if a != len(self.start_positions):
                    file.write(",")
